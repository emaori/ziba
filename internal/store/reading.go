...
const articleColumns = `
	...
	COALESCE(a.score, 0), COALESCE(a.base_score, 0), a.base_score IS NOT NULL, a.score_reason,
	COALESCE((SELECT f.direction FROM article_score_feedback f WHERE f.article_id = a.id), '')`
...
func scanArticle(row pgx.CollectableRow) (domain.Article, error) {
	...
	err := row.Scan(&a.ID, &a.SourceID, &a.SourceName, &a.URL, &a.Title, &a.Author,
		...
		&a.Score, &a.BaseScore, &a.BaseScoreSet, &a.ScoreReason, &a.ScoreFeedback)
	return a, err
}
...
func (s *Store) Article(ctx context.Context, id int64) (domain.Article, error) {
	...
	err := row.Scan(&a.ID, &a.SourceID, &a.SourceName, &a.URL, &a.Title, &a.Author,
		...
		&a.Categories, &a.Entities, &a.Tone,
		&a.Summary, &a.ContentQuality, &a.ContentQualityReason,
		&a.Score, &a.BaseScore, &a.BaseScoreSet, &a.ScoreReason, &a.ScoreFeedback, &a.FullText)
	if err != nil {
...
func collectArchivable(rows pgx.Rows, what string) ([]domain.Article, error) {
	articles, err := pgx.CollectRows(rows, func(row pgx.CollectableRow) (domain.Article, error) {
		...
		err := row.Scan(&a.ID, &a.SourceID, &a.SourceName, &a.URL, &a.Title, &a.Author,
			...
			&a.Summary, &a.ContentQuality, &a.ContentQualityReason,
			&a.Score, &a.BaseScore, &a.BaseScoreSet, &a.ScoreReason, &a.ScoreFeedback, &archived)
		if archived != nil {
